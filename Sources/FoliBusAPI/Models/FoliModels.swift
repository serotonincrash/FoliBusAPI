...
public enum Foli {
    ...
    // MARK: - Stop Model
    /// Information about a single stop
    public struct Stop: Codable, Sendable, Identifiable {
        /// The unique identifier for the stop
        public let id: String
        /// The human-readable name of the stop
        public let stopName: String
        
        public init(id: String, stopName: String) {
            self.id = id
...
/// Response containing all known stops
public struct FoliStopList: Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.stops = dictionary.map { (id, stopData) in
            Foli.Stop(id: id, stopName: stopData["stop_name"] ?? "")
        }.sorted { $0.id < $1.id }
    }
    
    public func encode(to encoder: Encoder) throws {
        var container = encoder.singleValueContainer()
        let dictionary = Dictionary(uniqueKeysWithValues: stops.map { ($0.id, ["stop_name": $0.stopName]) })
...
public struct FoliStopMonitoringResponse: Codable, Sendable {
    ...
    public let result: [FoliVehicleArrival]
    
    public init(sys: String, status: String, serverTime: TimeInterval, result: [FoliVehicleArrival]) {
        self.sys = sys
        self.status = status
...
// MARK: - Errors
public enum FoliAPIError: Error, LocalizedError, Sendable {
    case invalidURL
...
public struct CLLocationCoordinate2D: Sendable {
    ...
    /// Convert to CoreLocation's CLLocationCoordinate2D if available
